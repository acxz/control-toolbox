...
public:
    ...
    /*!
     * This computes matrices fxx, fxu, fuu, fx, and fu such that
     * \f[
     *  x_{n+1} = x_n^T fxx x_n + x_n^T fxu u_n + u_n^T fuu u_n + fx x_n fu u_n
     * \f]
     *
     * Note that the inputs x_next and subSteps are potentially being ignored
     * for 'true' discrete system models but are relevant for sensitivity
    ...
    virtual void getQuadratizedDynamics(const state_vector_t& x,
        ...
        state_matrix_t& fx,
        state_control_matrix_t& fu) = 0;

    void getQuadratizedDynamics(const state_vector_t& x,
        const control_vector_t& u,
        const int n,
        state_matrix_t& fxx,
        state_matrix_t& fxu,
    ...
    {
        getQuadratizedDynamics(x, u, x, n, 1, fxx, fxu, fuu, fx, fu);
    }
};
