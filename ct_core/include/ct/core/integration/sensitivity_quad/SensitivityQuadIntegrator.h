...
#include <ct/core/systems/continuous_time/quadratic/QuadraticSystem.h>
...
class SensitivityQuadIntegrator : public SensitivityQuad<STATE_DIM, CONTROL_DIM, SCALAR>
{
public:
     ...
     */
    SensitivityQuadIntegrator(const SCALAR dt,
        const std::shared_ptr<ct::core::QuadraticSystem<STATE_DIM, CONTROL_DIM, SCALAR>>& quadraticSystem,
    ...
    {
        setQuadraticSystem(quadraticSystem);
     ...
     */
    ~SensitivityQuadIntegrator() override = default;
    /**
     ...
     *             generation by setting a quadratic system, enabling state caching
     *             and settings up the function objects
     *
     * @param[in]  quadraticSystem  The quadratized system
     */
    virtual void setQuadraticSystem(
        const std::shared_ptr<ct::core::QuadraticSystem<STATE_DIM, CONTROL_DIM, SCALAR>>& quadraticSystem) override
    {
    ...
    /*!
	 * retrieve discrete-time quadratic system matrices fx and fu.
        ...
        state_control_matrix_t& fu) override
    {
        ...
        if (!timeVarying_)
        {
            Aconst_ = quadraticSystem_->getDerivativeState(x, u, n * dt_);
            Bconst_ = quadraticSystem_->getDerivativeControl(x, u, n * dt_);
        }

        integrateSensitivity(n, numSteps, fx, fu);
    };
...
private:
    ...
    std::shared_ptr<ct::core::quadraticSystem<STATE_DIM, CONTROL_DIM, SCALAR>> quadraticSystem_;
