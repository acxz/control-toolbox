...
HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::HPIPMInterfaceQQ()
    : N_(-1),
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::initializeAndAllocate()
{
    freeHpipmMemory();

    if (settings_.qqoc_solver_settings.qqoc_debug_print)
    {
    ...
    ::d_ocp_qp_ipm_arg_set_iter_max(&settings_.qqoc_solver_settings.num_qqoc_iterations, &arg_);
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::freeHpipmMemory()
{
    free(dim_mem_);
    free(qp_mem_);
    free(qp_sol_mem_);
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::configure(const NLOptConSettings& settings)
{
    settings_ = settings;
    ::d_ocp_qp_ipm_arg_set_iter_max(&settings_.qqoc_solver_settings.num_qqoc_iterations, &arg_);
}
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::solve()
{
// optional printout
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::computeStatesAndControls()
{
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::computeFeedbackMatrices()
{
    ...
    QQOCProblem<STATE_DIM, CONTROL_DIM>& p = *this->qqocProblem_;
    this->L_.resize(p.getNumberOfStages());

    for (int i = 1; i < this->qqocProblem_->getNumberOfStages(); i++)
    {
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::compute_lv()
{
    for (int i = 0; i < this->qqocProblem_->getNumberOfStages(); i++)
        ::d_ocp_qp_ipm_get_ric_k(&qp_, &arg_, &workspace_, i, this->lv_[i].data());
}

template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::printSolution()
{
#ifdef HPIPM_PRINT_MATRICES
    int ii;
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::setProblemImpl(
    std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem)
{
    // check if the number of stages N changed and adapt problem dimensions
    bool dimsChanged = changeProblemSize(qqocProblem);
...
template <int STATE_DIM, int CONTROL_DIM>
bool HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::configureInputBoxConstraints(
    std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem)
{
    ...
    for (int i = 0; i < N; i++)
    {
        if (nbu_[i] != qqocProblem->nbu_[i])
        {
            nbu_[i] = qqocProblem->nbu_[i];

            // set pointers to box constraint boundaries and sparsity pattern
            hlbu_[i] = qqocProblem->u_lb_[i].data();
            hubu_[i] = qqocProblem->u_ub_[i].data();
            hidxbu_[i] = qqocProblem->u_I_[i].data();
...
template <int STATE_DIM, int CONTROL_DIM>
bool HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::configureStateBoxConstraints(
    std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem)
{
    bool configChanged = false;
    ...
    for (int i = 0; i < N + 1; i++)
    {
        ...
        if (nbx_[i] != qqocProblem->nbx_[i])
        {
            nbx_[i] = qqocProblem->nbx_[i];

            // set pointers to box constraint boundaries and sparsity pattern
            hlbx_[i] = qqocProblem->x_lb_[i].data();
            hubx_[i] = qqocProblem->x_ub_[i].data();
            hidxbx_[i] = qqocProblem->x_I_[i].data();
...
template <int STATE_DIM, int CONTROL_DIM>
bool HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::configureGeneralConstraints(
    std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem)
{
    bool configChanged = false;
    ...
    hd_lg_0_Eigen_ = qqocProblem->d_lb_[0];  // - qqocProblem->C_[0] * x0; // uncommented since x0=0
    hd_ug_0_Eigen_ = qqocProblem->d_ub_[0];  // - qqocProblem->C_[0] * x0; // uncommented since x0=0

    for (int i = 0; i < N + 1; i++)  // (includes the terminal stage)
    {
        // get the number of constraints
        if (ng_[i] != qqocProblem->ng_[i])
        {
            ng_[i] = qqocProblem->ng_[i];
            configChanged = true;
        }

        qqocProblem->C_[i].resize(qqocProblem->ng_[i], STATE_DIM);
        qqocProblem->D_[i].resize(qqocProblem->ng_[i], CONTROL_DIM);
        qqocProblem->d_lb_[i].resize(qqocProblem->ng_[i], 1);
        qqocProblem->d_ub_[i].resize(qqocProblem->ng_[i], 1);
        ...
        {
            hlg_[i] = qqocProblem->d_lb_[i].data();
            hug_[i] = qqocProblem->d_ub_[i].data();
        }

        hC_[i] = qqocProblem->C_[i].data();
        hD_[i] = qqocProblem->D_[i].data();
    }
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::setupCostAndDynamics(StateMatrixArray& A,
...
template <int STATE_DIM, int CONTROL_DIM>
bool HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::changeProblemSize(std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> p)
{
    ...
    // resize the containers for the QQ cost approximation
    hQ_.resize(N_ + 1);
    hS_.resize(N_ + 1);
    hR_.resize(N_ + 1);
    hq_.resize(N_ + 1);
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::d_print_mat(int m, int n, double* A, int lda)
{
    int i, j;
    for (i = 0; i < m; i++)
    {
...
template <int STATE_DIM, int CONTROL_DIM>
void HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::d_print_e_tran_mat(int row, int col, double* A, int lda)
{
    int i, j;
    for (j = 0; j < col; j++)
    {
...
template <int STATE_DIM, int CONTROL_DIM>
const ct::core::ControlVectorArray<CONTROL_DIM>& HPIPMInterfaceQQ<STATE_DIM, CONTROL_DIM>::get_lv()
{
    return this->lv_;
}
