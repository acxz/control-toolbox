...
class HPIPMInterfaceQQ : public QQOCSolver<STATE_DIM, CONTROL_DIM>
{
public:
    ...
    virtual bool configureInputBoxConstraints(
        std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem) override;
    virtual bool configureStateBoxConstraints(
        std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem) override;

    //! brief setup and configure the general (in)equality constraints
    virtual bool configureGeneralConstraints(std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem) override;
...
private:
     ...
     */
    void setProblemImpl(std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem) override;

    /*!
     * @brief transcribe the problem for HPIPM
     *
     * See also the description of the QQOC Problem in class QQOCProblem.h
     *
     ...
     */
    bool changeProblemSize(std::shared_ptr<QQOCProblem<STATE_DIM, CONTROL_DIM>> qqocProblem);
