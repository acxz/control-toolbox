...
QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::QQOCProblem(int N) : nbu_(N, 0), nbx_(N + 1, 0)
{
    changeNumStages(N);
}

template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
bool QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::isConstrained() const
{
    return (isInputBoxConstrained() | isStateBoxConstrained() | isGeneralConstrained());
}
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
bool QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::isInputBoxConstrained() const
{
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
bool QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::isStateBoxConstrained() const
{
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
bool QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::isGeneralConstrained() const
{
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
int QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::getNumberOfStages()
{
    return K_;
}

template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::changeNumStages(int N)
{
    K_ = N;
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::setZero(const int& nGenConstr)
{
    fx_.setConstant(core::StateMatrix<STATE_DIM, SCALAR>::Zero());
    fu_.setConstant(core::StateControlMatrix<STATE_DIM, CONTROL_DIM, SCALAR>::Zero());
    fo_.setConstant(core::StateVector<STATE_DIM, SCALAR>::Zero());
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::setInputBoxConstraint(const int index,
    const int nConstr,
    ...
    const ct::core::ControlVector<CONTROL_DIM, SCALAR>& u_nom_abs)
{
        ...
        (sp(sp.rows() - 1) > (CONTROL_DIM - 1)))
    {
        ...
        throw(std::runtime_error("QQOCProblem setInputBoxConstraint: error in constraint config"));
    }

    if (index >= K_)
        throw(std::runtime_error("QQOCProblem cannot set an intermediate input Box constraint at time >= K_"));
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::setInputBoxConstraints(const int nConstr,
    const constr_vec_t& u_lb,
    const constr_vec_t& u_ub,
    const VectorXi& sp,
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::setIntermediateStateBoxConstraint(const int index,
    const int nConstr,
    ...
    const ct::core::StateVector<STATE_DIM, SCALAR>& x_nom_abs)
{
        ...
        (sp(sp.rows() - 1) > (STATE_DIM - 1)))
    {
        ...
        std::cout << "sparsity : " << sp.transpose() << std::endl;
        throw(std::runtime_error("QQOCProblem setIntermediateStateBoxConstraint: error in constraint config"));
    }

    if (index >= K_)
        throw(std::runtime_error("QQOCProblem cannot set an intermediate state Box constraint at time >= K_"));
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::setIntermediateStateBoxConstraints(const int nConstr,
    const constr_vec_t& x_lb,
    const constr_vec_t& x_ub,
    const VectorXi& sp,
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::setTerminalBoxConstraints(const int nConstr,
    const constr_vec_t& x_lb,
    const constr_vec_t& x_ub,
    const VectorXi& sp,
    const ct::core::StateVector<STATE_DIM, SCALAR>& x_nom_abs)
{
    if (nConstr > 0)
    {
            ...
            (sp(sp.rows() - 1) > (STATE_DIM - 1)))
        {
            ...
            std::cout << "sparsity : " << sp.transpose() << std::endl;
            throw(std::runtime_error("QQOCProblem setTerminalBoxConstraint: error in constraint config"));
        }
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::setGeneralConstraints(const constr_vec_t& d_lb,
    ...
    const constr_control_jac_t& D)
{
    if (d_lb.size() != d_ub.size() || C.rows() != D.rows() || d_lb.size() != C.rows())
    {
        ...
        throw(std::runtime_error("QQOCProblem setGeneralConstraints: error in constraint config"));
    }
...
template <int STATE_DIM, int CONTROL_DIM, typename SCALAR>
void QQOCProblem<STATE_DIM, CONTROL_DIM, SCALAR>::setFromTimeInvariantQuadraticQuadraticProblem(
    ...
    const double dt)
{
    ...
    fx_ = core::StateMatrixArray<STATE_DIM, SCALAR>(K_, A);
    fu_ = core::StateControlMatrixArray<STATE_DIM, CONTROL_DIM, SCALAR>(K_, B);
    fo_ = core::StateVectorArray<STATE_DIM, SCALAR>(K_ + 1, offset);
