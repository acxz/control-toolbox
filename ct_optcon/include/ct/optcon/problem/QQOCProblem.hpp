...
/*!
 * \brief Defines a Quadratic-Quadratic Optimal Control Problem, which is optionally constrained.
 *
 * This class defines a Quadratic Quadratic Optimal Control (QQOC) Problem in differential formulation, consisting of
 * - affine system dynamics
 * - QQ approximation of the cost function
 *
 * The unconstrained QQ problem has the following form: TODO: Edit documentation
 * \f[
 ...
 * \f]
 *
 * The constrained QQ problem additionally implements the box constraints
 ...
 * where  \hat \mathbf x_n and  \hat \mathbf u_n are current nominal/reference trajectories, around which the QQP is formed.
 *
...
public:
    ...
    //! returns the number of discrete time steps in the QQOCP, including terminal stage
    int getNumberOfStages();

    //! change the number of discrete time steps in the QQOCP
    ...
    /*!
     * \brief a convenience method which constructs an unconstrained QQOC Problem from an LTI system and continuous-time quadratic cost
     ...
     * @param stateOffset the offset for the affine system dynamics demanded by the QQOC Solver
     * @param dt the sampling time, required for discretization
     */
    void setFromTimeInvariantQuadraticQuadraticProblem(
    ...
    //! return a flag indicating whether this QQOC Problem is constrained or not
    ...
    ct::core::StateMatrixArray<STATE_DIM, SCALAR> fxx_;
    ct::core::StateMatrixArray<STATE_DIM, SCALAR> fxu_;
    ct::core::StateControlMatrixArray<STATE_DIM, CONTROL_DIM, SCALAR> fuu_;
    ct::core::StateMatrixArray<STATE_DIM, SCALAR> fx_;
    ct::core::StateControlMatrixArray<STATE_DIM, CONTROL_DIM, SCALAR> fu_;
    ct::core::StateVectorArray<STATE_DIM, SCALAR> fo_;

    //! constant term of in the QQ approximation of the cost function
    ct::core::ScalarArray<SCALAR> q_;

    //! QQ approximation of the pure state penalty, including terminal state penalty
    ct::core::StateVectorArray<STATE_DIM, SCALAR> qv_;
    ct::core::StateMatrixArray<STATE_DIM, SCALAR> Q_;

    //! QQ approximation of the pure control penalty
    ct::core::ControlVectorArray<CONTROL_DIM, SCALAR> rv_;
    ct::core::ControlMatrixArray<CONTROL_DIM, SCALAR> R_;

    //! QQ approximation of the cross terms of the cost function
...
private:
    //! the number of discrete time steps in the QQOCP, not including terminal stage
    int K_;
};
